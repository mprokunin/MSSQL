...
--~  
 ...
 --transaction_isolation_level, lock_timeout, deadlock_priority, row_count  
 --  
   ...
   (  
    SELECT  
     ...
     CASE sp.status  
      ...
      ELSE COALESCE(req.start_time, sp.login_time)  
     END AS start_time,  
     sp.dbid  
   ...
   '( ' +  
    ...
    '( ' +  
     'SELECT TOP(@i) ' +  
      ...
      'sp0.blocked, ' +  
      'CASE ' +  
       'WHEN sp0.hostprocess > '''' THEN CONVERT(BIT, 1) ' +  
       'ELSE CONVERT(BIT, 0) ' +  
      'END AS is_user_process ' +  
     'FROM ' +  
     '( ' +  
      'SELECT TOP(@i) ' +  
       'sp1.session_id, ' +  
      ...
      '( ' +  
       'SELECT TOP(@i) ' +  
        ...
        'MAX(CONVERT(VARCHAR(16), RTRIM(sp2.cmd)) COLLATE Latin1_General_Bin2) AS cmd, ' +  
     ...
     CASE  
      WHEN   
       (  
        ...
        'CONVERT(NVARCHAR(4000), NULL) AS block_info, ' +  
    ...
    '( ' +  
     'SELECT TOP(@i) ' +  
      ...
      '( ' +  
       ...
       '( ' +  
        'ms, ' +  
        '(1000 + NULLIF(s.is_user_process, 1)) * (DATEPART(ms, DATEADD(second, -(r.total_elapsed_time / 1000), GETDATE())) / 500) - DATEPART(ms, DATEADD(second, -(r.total_elapsed_time / 1000), GETDATE())), ' +  
        'DATEADD(second, -(r.total_elapsed_time / 1000), GETDATE()) ' +  
       '), ' +  
       'NULLIF(COALESCE(r.start_time, sp.login_time), CONVERT(DATETIME, ''19000101'', 112)), ' +  
       '( ' +  
        'SELECT TOP(1) ' +  
                  ...
                  '( ' +  
                   ...
                   'REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE( ' +  
                    ...
                    '( ' +  
                     ...
                     '( ' +  
                      'INT, ' +  
                      'SUBSTRING(wt.resource_description, NULLIF(CHARINDEX(N''dbid='', wt.resource_description), 0) + 5, COALESCE(NULLIF(CHARINDEX(N'' '', wt.resource_description, CHARINDEX(N''dbid='', wt.resource_description) + 5), 0), LEN(wt.resource_d
escription) + 1) - CHARINDEX(N''dbid='', wt.resource_description) - 5) ' +  
                     ') ' +  
                    '), ' +  
                    'NCHAR(31),N''?''),NCHAR(30),N''?''),NCHAR(29),N''?''),NCHAR(28),N''?''),NCHAR(27),N''?''),NCHAR(26),N''?''),NCHAR(25),N''?''),NCHAR(24),N''?''),NCHAR(23),N''?''),NCHAR(22),N''?''), ' +  
                    'NCHAR(21),N''?''),NCHAR(20),N''?''),NCHAR(19),N''?''),NCHAR(18),N''?''),NCHAR(17),N''?''),NCHAR(16),N''?''),NCHAR(15),N''?''),NCHAR(14),N''?''),NCHAR(12),N''?''), ' +  
...
rce_description) + 1) - 8) ' +  
            ...
            CASE  
             WHEN @statement_start_offset > 0 THEN  
              ...
              (  
               ...
               (  
                CASE                   WHEN @statement_end_offset = -1 THEN 2147483647  
  ...
  BEGIN;  
   ...
   CREATE UNIQUE CLUSTERED INDEX IX_SRD ON #locks (session_id, request_id, database_name);  
