...
/****** Object:  Job [ADM.ManagePartitions]    Script Date: 9/23/2020 7:36:29 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Notifies]]    Script Date: 9/23/2020 7:36:29 AM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Add New Partition]    Script Date: 9/23/2020 7:36:30 AM ******/
...
go
declare @path varchar(100) = ''Q'' + '':\renlog.data\REN_LOG_'', @partname1 char(8), @partname2 char(8), @exestr varchar(max) = '''', @i int = 2;
...
go', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Drop Partition 1]    Script Date: 9/23/2020 7:36:30 AM ******/
...
--select  (@EXESTR)
exec (@EXESTR)
select @EXESTR = ''alter database REN_LOG remove FILEGROUP REN_LOG_FG_'' + convert(char(8), dateadd(dd, -1, convert(datetime, @MIN_RANGE)), 112)
--select  (@EXESTR)
exec (@EXESTR)
go', 
