...
/*
---------------
-- Add distribution DB to AG here
---------------
...
-- Sync
:connect tsql03\SQL2008
BACKUP DATABASE [TDB] TO  
DISK = N'E:\BACKUP\TDB_20190508.BAK' WITH NOFORMAT, NOINIT,  NAME = N'TDB-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  compression, STATS = 10
GO

:connect tsql03\BO
USE [master];
RESTORE DATABASE [TDB] FROM  
DISK = N'E:\BACKUP\TDB_20190508.BAK' 
WITH  FILE = 1,  
MOVE N'TDB' TO N'E:\DATA\TDB.mdf',  
MOVE N'TDB_log' TO N'E:\DATA\TDB_log.ldf',  
NOUNLOAD,  STATS = 5
GO
...
--exec sp_droppullsubscription @publisher = N'testhadr', @publisher_db = N'TDB', @publication = N'TDB_Pub'
...
-- Failover to become primary replica
--exec sp_dropdistributor @no_checks = 1, @ignore_distributor = 1
EXEC sys.sp_adddistributor  
    @distributor = 'tsql03\bo',  
    @password = '12345678'; 
